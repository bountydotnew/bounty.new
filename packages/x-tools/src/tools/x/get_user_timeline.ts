import type {
  XGetUserTimelineParams,
  XTweetListResponse,
} from './types'

export const xGetUserTimelineTool = {
  id: 'x.get_user_timeline',
  description: 'Get tweets from a user timeline (including retweets)',
  request: {
    url: (params: XGetUserTimelineParams) => {
      ...
      return `https://api.x.com/2/users/${params.userId}/timelines/reverse_chronological?${queryParams.toString()}`
    },
    method: 'GET' as const,
    headers: (params: XGetUserTimelineParams) => ({
...
export type { XGetUserTimelineParams, XTweetListResponse }
