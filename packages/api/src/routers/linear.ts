...
import { getLinearBotInstallUrl } from '../lib/bot';
...
export const linearRouter = router({
  ...
  /**
   * Get the bot install URL for the current workspace detail page.
   * Returns null if bot credentials aren't configured.
   */
  getBotInstallUrl: orgProcedure
    .input(z.object({ workspaceId: z.string() }))
    .query(({ ctx, input }) => {
      const redirectPath = `/${ctx.org.slug}/integrations/linear/${input.workspaceId}`;
      return { url: getLinearBotInstallUrl(redirectPath) };
    }),
  ...
  syncWorkspace: orgProcedure.mutation(async ({ ctx }) => {
    ...
    try {
      ...
      // Create or update linear_account record — scoped to active org.
      // Uses onConflict because the unique index is (accountId, linearWorkspaceId)
      // but the lookup above also filters by organizationId, so reconnecting
      // with a different org would miss the existing row.
      await ctx.db
        ...
        .onConflictDoUpdate({
          target: [linearAccount.accountId, linearAccount.linearWorkspaceId],
          set: {
            linearUserId: user.id,
            linearWorkspaceName: workspace.name,
            linearWorkspaceUrl: workspace.url,
