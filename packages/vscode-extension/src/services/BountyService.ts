...
export class BountyService {
  ...
  private async fetchTRPC(
    ...
    if (!response.ok) {
      ...
      console.error(
        '[BountyService] TRPC request failed:',
        JSON.stringify({
    ...
    try {
      ...
      if (bounties.length > 0) {
        try {
          ...
          console.log(
            '[BountyService] Fetching stats for bounties:',
            bountyIds
          );

          const statsResponse = await this.fetchTRPC(
            'bounties.getBountyStatsMany',
            {
              bountyIds,
            }
          );
          ...
          console.log(
            '[BountyService] Bounties with stats:',
            JSON.stringify(
              bountiesWithStats.map((b) => ({
                id: b.id,
                title: b.title,
  ...
  async toggleBountyVote(
    bountyId: string
  ): Promise<{ voted: boolean; count: number }> {
    try {
      const response = await this.fetchTRPC(
        'bounties.voteBounty',
        { bountyId },
        true
      );
  ...
  async toggleBountyBookmark(
    bountyId: string
  ): Promise<{ bookmarked: boolean }> {
    try {
      const response = await this.fetchTRPC(
        'bounties.toggleBountyBookmark',
        { bountyId },
        true
      );
      return response;
