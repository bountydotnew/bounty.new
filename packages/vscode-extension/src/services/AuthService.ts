...
export class AuthService {
  ...
  private async loadSession(): Promise<void> {
    ...
    console.log(
      '[AuthService] Loading session from storage:',
      JSON.stringify({
  ...
  async requestDeviceCode(): Promise<DeviceCodeResponse> {
    try {
      ...
      if (error instanceof Error && error.message.includes('fetch')) {
        ...
        throw new Error(
          `Network error: Cannot reach ${API_CONFIG.authBaseUrl}. Check your connection.`
        );
      }
      throw error;
    }
  ...
  async pollForToken(deviceCode: string): Promise<TokenResponse> {
    try {
      ...
      try {
        ...
        console.error(
          '[AuthService] Failed to parse token response:',
          responseText
        );
        data = {
  ...
  private evaluateTokenResponse(tokenResponse: TokenResponse): {
    ...
    switch (error) {
      ...
      default:
        return {
          outcome: 'error',
          message: tokenResponse.error_description || 'Unknown error',
        };
    }
  }
  ...
  private async handlePollStep(
    ...
    resolveValue?: { success: boolean; error?: string };
    newInterval?: number;
  }> {
    ...
    switch (evalResult.outcome) {
      ...
      case 'error':
        return {
          ...
          resolveValue: {
            success: false,
            error: evalResult.message || 'Unknown error',
          },
        };
      case 'continue':
      default:
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (step.shouldResolve) {
            resolve(
              step.resolveValue ?? { success: false, error: 'Unknown error' }
            );
            return;
          }
  ...
  async getAuthHeader(): Promise<Record<string, string>> {
    ...
    if (!session) {
      console.warn(
        '[AuthService] No session found, returning empty auth headers'
      );
      return {};
    }
