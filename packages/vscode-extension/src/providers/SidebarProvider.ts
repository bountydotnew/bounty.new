...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (data: WebviewMessage) =>
      this.handleMessage(data, webviewView.webview)
    );
  }
  ...
  private async handleMessage(
    ...
    try {
      switch (message.type) {
        ...
        case 'fetchBounties':
          await this.handleFetchBounties(
            webview,
            message.params as FetchBountiesParams
          );
          break;

        case 'fetchBountyDetail':
          await this.handleFetchBountyDetail(
            webview,
            message.bountyId as string
          );
          break;
        ...
        case 'toggleCommentLike':
          await this.handleToggleCommentLike(
            webview,
      ...
      webview.postMessage({
        ...
        message:
          error instanceof Error
            ? error.message
            : 'Failed to fetch bounty details',
      });
    }
  }
