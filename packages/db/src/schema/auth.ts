...
export const session = pgTable(
  ...
  (t) => [
    // Speeds up user cascade deletes and session lookups by user
    index('session_user_id_idx').on(t.userId),
    // Speeds up expired session cleanup queries
...
export const account = pgTable(
  ...
  (t) => [
    // Speeds up account lookups by user (used on every auth flow)
