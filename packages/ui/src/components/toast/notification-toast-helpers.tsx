...
import {
	...
	type BountyLikedToastProps,
} from "./bounty-liked-toast";
import {
	showBountyCommentToast,
	type BountyCommentToastProps,
} from "./bounty-comment-toast";
import {
	showBountyReportedToast,
	type BountyReportedToastProps,
} from "./bounty-reported-toast";
	...
	switch (type) {
		case "bounty_comment": {
			...
			const commentProps = data.onMarkAsRead
				...
				: baseProps;
			return showBountyCommentToast(
				commentProps as BountyCommentToastProps,
				toastOptions,
			);
		}
		...
		case "bounty_voted": {
			...
			const likedProps = data.onMarkAsRead
				? { ...baseProps, onMarkAsRead: data.onMarkAsRead }
				: baseProps;
			return showBountyLikedToast(
				likedProps as BountyLikedToastProps,
				toastOptions,
			);
		}
		...
		case "bounty_reported": {
			...
			const reportedProps = data.onMarkAsRead
				? { ...baseProps, onMarkAsRead: data.onMarkAsRead }
				: baseProps;
			return showBountyReportedToast(
				reportedProps as BountyReportedToastProps,
				toastOptions,
			);
		}
