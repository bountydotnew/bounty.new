...
function Button({
  ...
  // Determine haptic type based on variant
  const hapticType =
  ...
  const defaultProps: Record<string, unknown> = {
    ...
    onClick: () =>
      haptic.trigger(hapticType as Parameters<typeof haptic.trigger>[0]),
  };
...
function HotkeyButton({
  ...
  return (
    <Button className={cn('pr-2 pl-3', className)} size={size} {...props}>
      <span className="mr-1">{children}</span>
      <kbd className="ml-auto inline-flex items-center gap-1 rounded-md border border-border bg-surface-2 px-1.5 font-medium text-[15px]">
        {hotkey}
      </kbd>
    </Button>
  );
...
export type { ButtonProps, HotkeyButtonProps };
