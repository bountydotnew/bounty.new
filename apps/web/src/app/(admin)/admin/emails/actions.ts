...
export async function sendTestEmailAction(input: {
  ...
  const res = await sendEmail({
    ...
    react:
      input.template === 'alpha' ? AlphaAccessGranted({ name: '' }) : undefined,
    text: input.text ?? '',
  });
  return { id: res.data?.id ?? null, error: res.error?.message ?? null };
}

export async function subscribeAudienceAction(input: {
  email: string;
...
export async function unsubscribeAudienceAction(input: {
  email: string;
  audienceKey: AudienceKey;
}) {
