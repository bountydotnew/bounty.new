...
export default function MembersPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Invite section — only for owners of non-personal teams */}
            ...
            {members.map((m) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <Avatar className="h-8 w-8 rounded-full shrink-0">
                    {m.user.image && (
                      <AvatarImage alt={m.user.name ?? ''} src={m.user.image} />
                    )}
                    <AvatarFacehash
                      name={m.user.handle ?? m.user.email}
                      size={32}
                    />
                  </Avatar>
                  <div className="min-w-0">
                    ...
                    <span className="text-xs text-text-muted truncate block">
                      {m.user.handle ? `@${m.user.handle}` : m.user.email}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {isOwner && m.userId !== currentUserId && !isPersonalTeam && (
                    <DropdownMenu>
