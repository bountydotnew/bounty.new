...
import { SimpleBountySkeleton } from "./skeletons/bounty-skeleton";
import { LOADING_SKELETON_COUNTS, PAGINATION_LIMITS } from "@/constants/dashboard";
...
export const MyBountiesSidebar = memo(function MyBountiesSidebar({
  ...
  const displayBounties = myBounties?.slice(0, PAGINATION_LIMITS.MY_BOUNTIES_SIDEBAR);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {isLoading && (
          <SimpleBountySkeleton count={LOADING_SKELETON_COUNTS.MY_BOUNTIES} />
        )}

        {!isLoading && displayBounties && displayBounties.length > 0 && (
          displayBounties.map((bounty, index) => (
            <div key={bounty.id}>
              <div 
                ...
                aria-label={onBountyClick ? `View bounty: ${bounty.title}` : undefined}
              >
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium truncate pr-2" title={bounty.title}>
                    {bounty.title}
                  </h4>
                  <Badge
              ...
              {index < displayBounties.length - 1 && <Separator className="mt-4" />}
            </div>
          ))
        )}
