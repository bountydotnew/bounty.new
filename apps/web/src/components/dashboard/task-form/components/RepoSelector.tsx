                ...
                >
                    {selectedRepository ? (
                        <>
                            ...
                            <ChevronDown className={`w-3 h-3 transition-transform ${showRepoDropdown ? 'rotate-180' : ''}`} />
                        </>
                    ) : (
                        <>
                            <span className="text-[14px] leading-[150%] tracking-[-0.02em] font-medium">Select repositories</span>
                            <ChevronDown className={`w-3 h-3 transition-transform ${showRepoDropdown ? 'rotate-180' : ''}`} />
                        </>
                    )}
                </button>
                        ...
                        {reposLoading ? (
                            ...
                            filteredRepositories.map((repo: string) => (
                                ...
                                >
                                    ...
                                    {selectedRepository === repo && <Check className="w-3 h-3 text-green-500 ml-auto shrink-0" />}
                                </DropdownMenuItem>
                            ))
                        ) : (
                            <div className="px-2 py-1.5 text-sm text-[#5A5A5A]">
