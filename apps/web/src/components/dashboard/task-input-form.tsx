...
// Hooks
import { useRepositories } from '@bounty/ui/hooks/use-repositories';
import { useBranches } from '@bounty/ui/hooks/use-branches';
import { useIssues } from '@bounty/ui/hooks/use-issues';
...
export const TaskInputForm = forwardRef<TaskInputFormRef, TaskInputFormProps>(
  ({ placeholder = 'Create a new bounty...' }, ref) => {
    ...
    const githubUsername = (
      userData as { data?: { profile?: { githubUsername?: string } } }
    ...
    const [_showImportPrompt, setShowImportPrompt] = useState(false);
    ...
    const _handleImportIssue = async () => {
      if (!selectedIssue) {
        return;
      }
      setIsImporting(true);
    ...
    return (
      <div className="flex w-full shrink-0 flex-col px-4 lg:max-w-[805px] xl:px-0 mx-auto min-w-0">
        <form
          className="w-full flex flex-col mt-10 mb-6 min-w-0"
          onSubmit={onSubmit}
        >
