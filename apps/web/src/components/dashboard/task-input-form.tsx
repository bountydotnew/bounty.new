...
export const TaskInputForm = forwardRef<TaskInputFormRef, TaskInputFormProps>(({
    ...
    // Reset selected repository when repositories change (e.g., after account switch)
    // Only runs when repositories array reference changes, avoiding unnecessary checks
    const prevRepositoriesRef = useRef<string[]>([]);
    useEffect(() => {
        // Only check if repositories actually changed and selection is invalid
        const reposChanged = 
        ...
        if (reposChanged && selectedRepository && !repositories.includes(selectedRepository)) {
            console.log('[TaskInputForm] Selected repository no longer available, resetting');
