...
function IssueHeader({ commentSent }: { commentSent: boolean }) {
  return (
      ...
      <div className="flex-1 border border-gh-border rounded-md">
        <div className="bg-gh-surface px-4 py-2 border-b border-gh-border flex items-center justify-between">
          <div className="text-sm text-gh-text">
            <Link href="/bounties" className="font-semibold hover:text-gh-link">
              ripgrim
            </Link>
...
function ContributorComment({ randomDev }: { randomDev: string }) {
  return (
    <div className="flex gap-3">
      <div className="shrink-0">
        <Image
            ...
            <div className="text-xs text-gh-text-muted">
              {commentSent && botReacted && (
                <span className="inline-flex items-center gap-1">
                  ...
                  <span className="text-gh-link">@bountydotnew</span> reacted
                </span>
              )}
            </div>
            <button
              ...
              className={`bg-gh-success text-white px-4 py-1.5 rounded-md text-sm font-medium hover:bg-success disabled:opacity-50 disabled:cursor-not-allowed transition-colors ${
                commentSent ? '' : 'animate-pulse shadow-md shadow-success/40'
              }`}
            >
...
function BotSubmissionReply({ randomDev }: { randomDev: string }) {
  return (
      ...
      <div className="flex-1 border border-gh-border rounded-md">
        ...
        <div className="p-4 text-sm text-gh-text">
          <p>
            ...
            <Link href="/contributors" className="text-gh-link hover:underline">
              @{randomDev}
            </Link>
...
function GitHubIssuePage() {
  ...
  return (
    <div className="bg-gh-bg h-full overflow-auto" ref={scrollRef}>
      <IssueHeader commentSent={commentSent} />
      <div className="px-6 py-6 space-y-4">
        <OriginalIssueComment />
        <BotBountyComment />
        <ContributorComment randomDev={randomDev} />
        <SubmitCommandArea
          randomDev={randomDev}
        ...
        />
        {botReplied && <BotSubmissionReply randomDev={randomDev} />}
      </div>
    </div>
  );
}
