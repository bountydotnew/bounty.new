...
export const Markdown = ({ children, className }: MarkdownProps) => {
  const { displayMd, attachments } = React.useMemo(() => {
    ...
    const atts: string[] = [];
    const isImgUrl = (t: string) =>
      /\.(png|jpe?g|gif|webp|svg)(\?.*)?$/i.test(t);
    const isGhAttachment = (t: string) =>
      /^https?:\/\/github\.com\/user-attachments\/assets\/[A-Za-z0-9-]+$/i.test(
        t,
        ...
        components={{
          ...
          p: ({ children, ...props }) => {
            ...
            if (typeof only === "string") {
              ...
              const isImageUrl = /\.(png|jpe?g|gif|webp|svg)(\?.*)?$/i.test(
                text,
              );
              const isGithubAttachment =
                ...
                /^https?:\/\/user-attachments\.githubusercontent\.com\//i.test(
                  text,
                );
              if (isImageUrl) {
                return (
