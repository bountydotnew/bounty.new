...
const isVideoUrl = (t: string) =>
  /\.(mp4|webm|ogg)(\?.*)?$/i.test(t) ||
  /^https?:\/\/github\.com\/user-attachments\/assets\//i.test(t) ||
  /^https?:\/\/user-attachments\.githubusercontent\.com\//i.test(t);
  ...
  for (const node of tree.children || []) {
    switch (node.type) {
      case "heading":
        out.push({
          type: "heading",
          depth: Math.min(6, Math.max(1, node.depth)) as any,
          text: textFrom(node),
        });
        break;
      case "thematicBreak":
      ...
      case "paragraph": {
        ...
        if (node.children?.length === 1) {
          ...
          if (c.type === "link" && typeof c.url === "string") {
            ...
            if (isImageUrl(href)) {
              out.push({ type: "image", url: href });
              break;
            }
            if (isVideoUrl(href)) {
              out.push({ type: "video", url: href });
              break;
            }
          }
          if (c.type === "text" && typeof c.value === "string") {
            ...
            if (isImageUrl(t)) {
              out.push({ type: "image", url: t });
              break;
            }
            if (isVideoUrl(t)) {
              out.push({ type: "video", url: t });
              break;
            }
          }
        }
      ...
      case "image":
        if (typeof node.url === "string")
          out.push({ type: "image", url: node.url, alt: node.alt });
        break;
      case "link":
        if (typeof node.url === "string") {
          if (isImageUrl(node.url)) {
            out.push({ type: "image", url: node.url });
            break;
          }
          if (isVideoUrl(node.url)) {
            out.push({ type: "video", url: node.url });
            break;
          }
        }
        out.push({ type: "paragraph", text: textFrom(node) });
...
export function partitionAttachments(blocks: MarkdownBlock[]): {
  main: MarkdownBlock[];
  ...
  for (const b of blocks) {
    if (b.type === "image" || b.type === "video")
      attachments.push(b as MediaBlock);
    else main.push(b);
  }
