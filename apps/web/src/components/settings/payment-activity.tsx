...
import { Loader2, ArrowUpRight, ArrowDownLeft, Clock, AlertCircle, ExternalLink } from 'lucide-react';
...
function getStatusDisplay(status: PayoutStatus) {
  switch (status) {
    case 'completed':
      return <span className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-emerald-500" /><span className="text-sm">Completed</span></span>;
    case 'processing':
      return <span className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-blue-500 animate-pulse" /><span className="text-sm">Processing</span></span>;
    case 'pending':
      return <span className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-muted-foreground" /><span className="text-sm text-muted-foreground">Pending</span></span>;
    case 'failed':
      return <span className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-red-500" /><span className="text-sm">Failed</span></span>;
    default:
...
function getTypeIcon(type: ActivityType) {
  switch (type) {
    case 'payout':
      return { icon: ArrowDownLeft, color: 'text-emerald-600', label: 'Received' };
    case 'created':
...
export function PaymentActivity() {
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center p-16 text-center">
        ...
        <h3 className="text-base font-semibold mb-2">Failed to load activity</h3>
  ...
  const createdCount = activities.filter((a: any) => a.type === 'created').length;
            ...
            {activities.map((activity: any) => {
              ...
              return (
                <TableRow key={activity.id}>
                  <TableCell>
                    <div className={cn('flex items-center gap-2', typeConfig.color)}>
                  ...
                  <TableCell>
                    <div className="max-w-[240px]">
                      <p className="text-sm font-medium truncate" title={activity.bounty?.title || 'Unknown bounty'}>
      ...
      <div className="flex items-start gap-3 p-4 rounded-lg border bg-muted/30 text-sm">
        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Once your PR is merged and approved, payouts are released within 2-3 business days while funds clear from the original payment.
          </p>
        </div>
      </div>
    </div>
