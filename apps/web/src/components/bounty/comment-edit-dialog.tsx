  ...
  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="bottom" className="w-full max-h-[70vh] overflow-y-auto p-4 rounded-t-2xl">
          <div className="text-lg font-semibold leading-none tracking-tight mb-3">Edit comment</div>
          ...
          <div className="mt-4 flex items-center justify-end gap-2">
            ...
            <Button size="sm" onClick={() => { const trimmed = value.trim(); if (!trimmed || trimmed.length > maxChars) return; onSave(trimmed); }} className="rounded-md" disabled={isSaving || value.trim().length === 0}>Save</Button>
          </div>
        </SheetContent>
      </Sheet>
    );
