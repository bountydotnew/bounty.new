...
/**
 * BountiesFeed List View
 *
 * Explicit variant for displaying bounties in a list layout.
 * This replaces the `layout="list"` boolean prop pattern.
 *
 * @example
 * ```tsx
...
export function ListView() {
  ...
  if (!context) {
    throw new Error('ListView must be used within BountiesFeedProvider');
  }
  ...
  if (isLoading) {
    return (
      <div className={className}>
        ...
        <BountySkeleton count={LOADING_SKELETON_COUNTS.BOUNTIES} grid={false} />
      </div>
    );
  }
