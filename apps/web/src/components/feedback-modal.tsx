...
export function FeedbackModal() {
  const { isFeedbackOpen, closeFeedback, selectedElement, config } = useFeedback()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (includeScreenshot) {
        ...
        const overlayElement = document.getElementById("feedback-overlay-layer")
        if (overlayElement) {
          overlayElement.style.display = "none"
        }

        // Apply privacy mask
        const sensitiveElements = document.querySelectorAll('[data-privacy="masked"]')
      ...
      config.onFeedbackSubmit?.({ comment, route: window.location.href, selectedElement: selectedElement?.toString() || "", includeScreenshot: includeScreenshot.toString() })
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[#232323]">
          ...
          <button type="button" onClick={closeFeedback} className="text-[#929292] hover:text-white transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {status === "success" ? (
          <div className="p-8 flex flex-col items-center text-center">
            ...
            <h3 className="text-white font-medium text-lg mb-2">Feedback Sent!</h3>
            <p className="text-[#929292] text-sm">Thank you for helping us improve.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              <label htmlFor="comment" className="text-[#929292] text-sm font-medium">Describe the issue</label>
              <textarea
                id="comment"
                value={comment}
                ...
                style={{
                  borderColor: status === "sending" ? ui.primaryColor : undefined,
                }}
                required
              />
            </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-[#141414] rounded-lg border border-[#232323]">
              ...
              />
              <label htmlFor="include-screenshot" className="text-[#929292] text-sm font-medium cursor-pointer flex-1">
                Include screenshot
              </label>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                ...
                {status === "sending" ? "Sending..." : ui.submitLabel as string}
              </button>
            </div>
          </form>
        )}
