  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (includeScreenshot) {
        // Hide the modal temporarily so it doesn't block the screenshot
      ...
      config.onFeedbackSubmit?.({ comment, route: window.location.href, selectedElement, includeScreenshot })
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[#232323]">
          ...
          <button onClick={closeFeedback} className="text-[#929292] hover:text-white transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {status === "success" ? (
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-[#929292] text-sm font-medium">Describe the issue</label>
              <textarea
                value={comment}
            ...
            <div className="flex items-center gap-3 p-3 bg-[#141414] rounded-lg border border-[#232323]">
              <input
                ...
                style={{ accentColor: ui.primaryColor }}
              />
