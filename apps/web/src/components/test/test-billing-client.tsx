...
function ActiveSubscriptionsCard({ subscriptions }: { subscriptions: any[] }) {
  return (
    <Card>
      <CardHeader>
...
function AvailableProductsCard({ products }: { products: any[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Available Products</CardTitle>
        <CardDescription>
          All available products you can subscribe to
        </CardDescription>
      </CardHeader>
      <CardContent>
...
function UsageTrackTestCard({
  ...
  setFeatureId: (value: string) => void;
  trackValue: number;
  setTrackValue: (value: number) => void;
  trackResult: any;
  onTrack: () => void;
}) {
  return (
...
export function TestBillingClient() {
  ...
  const { customer, isLoading, refetch, check, track } = useCustomer();
  ...
  const subscriptions = products.filter(
    (p: any) => p.subscription_ids && p.subscription_ids.length > 0
  );
  ...
  const feeScenarios = [
    ...
    {
      monthlySpend: 100,
      expected: 'Basic ($10/mo): $500 allowance, 0% fee, unlimited bounties',
    },
    {
      monthlySpend: 1000,
      expected: 'Pro ($25/mo): $5,000 allowance, 2% fee, unlimited bounties',
    },
    {
      monthlySpend: 10_000,
      expected: 'Pro+ ($150/mo): $12,000 allowance, 4% fee, unlimited bounties',
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <ActiveSubscriptionsCard subscriptions={subscriptions} />

      <FeaturesStatusCard features={features} />

      <AvailableProductsCard products={products} />

      <FeatureCheckTestCard
        featureId={featureId}
        ...
        <CardContent>
          <div className="space-y-3">
            {feeScenarios.map((scenario, i) => (
              <div
                key={scenario.monthlySpend}
                className="rounded-lg border p-3"
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium">
                    ${scenario.monthlySpend}/mo spend
                  </span>
