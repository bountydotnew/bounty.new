	...
	return [
		new Builder()
			...
			.addSubcommand((subcommand) =>
				subcommand.setName("create").setDescription("Create a new bounty"),
			)
			.addSubcommand((subcommand) =>
				subcommand
					.setName("list")
			...
			.addSubcommand((subcommand) =>
				subcommand
					...
					.addStringOption((option) =>
						option.setName("id").setDescription("Bounty ID").setRequired(true),
					),
			)
			.addSubcommand((subcommand) =>
				subcommand
...
async function handleCreate(
	...
	const instance = await Runtime.runPromise(runtime)(
		reacord.reply(
			...
			<Container>
				<TextDisplay>
					Click the button below to create a new bounty:
				</TextDisplay>
				<ActionRow>
					<ModalButton
						label="Create Bounty"
							...
							try {
								...
								if (modalInteraction.user.id !== interaction.user.id) {
									await modalInteraction.reply({
										content:
											"❌ You are not authorized to submit this form. Please use `/bounty create` yourself.",
										flags: MessageFlags.Ephemeral,
									});
									return;
								}
